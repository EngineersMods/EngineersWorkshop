...
public class Transfer {
    ...
    private engineers.workshop.gui.page.setting.ItemSetting[] items;
    private boolean useWhiteList;


    public Transfer(boolean isInput) {
        this.isInput = isInput;
        items = new engineers.workshop.gui.page.setting.ItemSetting[engineers.workshop.gui.page.setting.ItemSetting.ITEM_COUNT];
        for (int i = 0; i < items.length; i++) {
            items[i] = new engineers.workshop.gui.page.setting.ItemSetting(i);
        }
    }
    ...
    public engineers.workshop.gui.page.setting.ItemSetting getItem(int id) {
        return items[id];
    }
    ...
    public boolean isValid(TileTable table, ItemStack item) {
        ...
        for (engineers.workshop.gui.page.setting.ItemSetting itemSetting : items) {
    ...
    public void writeToNBT(NBTTagCompound compound) {
        ...
        if (enabled) {
            ...
            for (int i = 0; i < items.length; i++) {
                engineers.workshop.gui.page.setting.ItemSetting item = items[i];
                if (item.getItem() != null) {
    ...
    public void readFromNBT(NBTTagCompound compound) {
        for (engineers.workshop.gui.page.setting.ItemSetting item : items) {
            item.setItem(null);
            item.setMode(engineers.workshop.gui.page.setting.TransferMode.PRECISE);
        }
        ...
        if (enabled) {
            ...
            if (compound.hasKey(NBT_ITEMS)) {
                ...
                for (int i = 0; i < itemList.tagCount(); i++) {
                    ...
                    engineers.workshop.gui.page.setting.ItemSetting itemSetting = items[id];
                    itemSetting.setMode(engineers.workshop.gui.page.setting.TransferMode.values()[itemCompound.getByte(NBT_MODE)]);
