...
public enum Upgrade {
	...
	Upgrade(MaxCount maxCount, ParentType type) {
		this(maxCount, type == null || type == ParentType.NULL ? EnumSet.noneOf(ParentType.class)
				...
				? EnumSet.of(ParentType.CRAFTING, ParentType.SMELTING)
				: EnumSet.of(type)));
	}
	...
	public void addInfo(List<String> info) {
		...
		if (GuiScreen.isShiftKeyDown()) {
			if (getMaxCount() == 1)     info.add(I18n.format(MODID + ":" + "upgrade" + "." + "unstackable"));
			else if (getMaxCount() > 1) info.add(I18n.format(MODID + ":" + "upgrade" + "." + "stackable", getMaxCount()));
	...
	public boolean isValid(ItemStack parent) {
		for (ParentType validParent : validParents) if (!validParent.isValidParent(parent)) return false;
